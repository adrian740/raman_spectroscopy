...
%
x_start = -60  % start corrdinate from first file (microns)
x_inc = 0.6       % increment (microns)
for ii = 1:length(FF)
...
max_amp_pei = mean(amp_pei(x_coord>-60 & x_coord<-47))
min_amp_epo = mean(amp_epo(x_coord>-60 & x_coord<-57.6))

min_amp_pei = mean(amp_pei(x_coord>50 & x_coord<98.0))
max_amp_epo = mean(amp_epo(x_coord>50 & x_coord<60))
...
hold on;
plot (x_coord + 26.5, relamp_pei, x_coord + 26.5, relamp_pei_smooth);
...
ylabel ('peak amplitude normalized concentration');
hold on;
plot (x_coord + 26.5, relamp_pei_smooth);
legend ('% Epoxy','% PEI','location','best');
